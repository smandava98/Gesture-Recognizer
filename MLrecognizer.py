...
"""
Randomly generates bounding boxes 
Returns hog vector with labels
Label : 1 if hand ,0 otherwise 
"""
def hand(frame,imgset):
    one =[]
    two =[]
    ...
    for nameimg in frame.image:
        ...
        one.append(convertToGrayToHOG(crop(imgset[nameimg],x_tl,x_tl+side,y_tl,y_tl+side)))
        while dic[0] <= 1 or dic[1] < 1:
            ...
            if dic[0]== 1:
                break
    label_1 = [1 for i in range(0,len(one)) ]
    label_0 = [0 for i in range(0,len(two))]
...
"""
def negativeMine(cached_window,frame, imgset, model, step_x, step_y):   
    lis = []
...
def improve_Classifier_using_HNM(hog_list, label_list, frame, imgset, threshold=50, max_iterations=25): # frame - bounding boxes-df; yn_df - yes_or_no df
    ...
    no_of_false_positives = 14943210     # Random high value
    i = 0
    ...
    while True:
        ...
        ret = negativeMine(cached_wind,frame, imgset, model, step_x=step_x, step_y=step_y)
...
def image_pyramid_step(model, img, scale=1.0):
    ...
    for x in range(0,x_border-side,32):
        for y in range(0,y_border-side,24):
            ...
            if confidence[0][1] > max_confidence_seen:  #implement confidence interval
...
class GestureRecognizer(object):
    ...
    def train(self, train_list):
        ...
        # Classify the various signs/hand gestures
...
def min_suppress(boxes, overlapThresh):
...
def main():
    gs = GestureRecognizer('/Users/saimandava/Desktop/Gesture Recognizer/dataset')   #can replace line with own directory
    userlist=[ 'user_3', 'user_4','user_5','user_6','user_7','user_9','user_10']      #can replace line with own dataset
